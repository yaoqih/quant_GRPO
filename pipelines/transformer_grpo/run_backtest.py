...
def main() -> None:
    ...
    if args.checkpoint is None:
        ...
        ckpt_root = Path(train_cfg.get("checkpoint_root", "runs/transformer_grpo"))
        candidates = sorted(ckpt_root.glob("*/best.pt"))
        if not candidates:
            raise ValueError("Must specify a checkpoint with --checkpoint when no saved run is available.")
        args.checkpoint = max(candidates, key=lambda path: path.stat().st_mtime)
        print(f"[Backtest] Auto-selected latest checkpoint at {args.checkpoint}")
    ...
    top_k = args.top_k if args.top_k is not None else int(backtest_cfg.get("top_k", 10))
    min_weight = args.min_weight if args.min_weight is not None else float(backtest_cfg.get("min_weight", 0.0))
    temperature = args.temperature if args.temperature is not None else float(backtest_cfg.get("temperature", 1.0))
